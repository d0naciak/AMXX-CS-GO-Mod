...
public plugin_init() {
	register_plugin("CSGO: Super VIP", "1.0.0", "d0naciak.pl");
	...
	g_pCvarFlagaVip = register_cvar("csgo_flagasvip", "s");
	g_pCvarIleHpFrag = register_cvar("csgo_hpzafragasvip", "8");
	g_pCvarIleHpFragHs = register_cvar("csgo_hpzafragahssvip", "160");
	...
	register_clcmd("say /svip", "cmd_VipInfo");
	register_clcmd("say /svips", "cmd_VipyOnline");
...
public fw_Smierc_Post(id, iAtt, iShGb) {
	...
	set_user_health(iAtt, min(110, iBonus));
...
MenuBroni(id) {
	...
	menu_additem(iMenu, "GALIL + Deagle");
...
public cmd_WybierzTeam(id) {
	if(is_user_vip(id) && !g_bVipPrzywitany[id]) {
		...
		show_hudmessage(id, "Super VIP %s wszedl na serwer!", szNick);
...
public cmd_VipInfo(id) {
	show_motd(id, "addons/amxmodx/configs/sklepsms/svip.txt", "Opis Super VIPa");
...
public cmd_VipyOnline(id) {
	...
	iLen = formatex(szWiadomosc, 511, "\ySuper VIPy Online:^n");
	...
	if(!strlen(szNick)) {
		iLen += formatex(szWiadomosc[iLen], 511 - iLen, "\rnie ma zadnego Super VIPa!");
	}
